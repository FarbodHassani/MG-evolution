...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, icsettings & ic)
{
  ...
  sim.smooth_delta=1;
    ...
    {
      COUT << " gravity theory set to: " << COLORTEXT_CYAN << "Newtonian Modified gravity: nDGP" << COLORTEXT_RESET << endl;
      sim.gr_flag = 0;
      sim.MG_nDGP_Newtonian_flag = 1;
    }
	...
	else
	{
      ...
      COUT << " smoothing density: " << COLORTEXT_CYAN << "Yes" << COLORTEXT_RESET << endl;
      sim.smooth_delta = 1;
      if (!parseParameter(params, numparam, "R_smoothing", cosmo.R_smoothing))
    	{
    		cosmo.R_smoothing = 1.0;
    	}
      COUT << " smoothing radius: " << COLORTEXT_CYAN << cosmo.R_smoothing << COLORTEXT_RESET << endl;
