...
//////////////////////////
// Description:
//   construction of real-space Modified Gravity source field for Fourier-based solvers
//   density(k) = density(k) * Gaussian(k)
//   Arguments:
//   source     reference to delta field (rescaled by a^3) in Newtonian simulation
//   R_smoothing   radius of smoothing
//
//////////////////////////
...
// Description:
//   construction of real-space Modified Gravity source field for Fourier-based solvers
//   source_MG = source * (1+\DeltaG_eff/G)
//   Arguments:
//   source     reference to fully dressed source field (rescaled by a^3)
//   bgmodel    background model of the source (rescaled by a^3) to be rescaled to find delta from delta_rho
// Returns:
//
//////////////////////////
...
{
	Site x(source_mg.lattice());
  double DeltaG_over_G, beta;
  beta  = 1. + (4./3./a) * (H_conf/H0) * H0rc * (1. + H_conf_prime/2./ (H_conf * H_conf) );
	for (x.first(); x.test(); x.next())
	{
    DeltaG_over_G = 1./3./beta;
...
template <class FieldType>
void Modified_Gravity_Newtonian_nDGP_screened(Field<FieldType> & source_mg, Field<FieldType> & source ,Field<FieldType> & source_smooth, const double H0rc, const double a, const double H_conf , const double H0, const double H_conf_prime, const double fourpiG, const double Omega_m0, const double dx, double const num_pts)
{
	Site x(source_mg.lattice());
  double DeltaG_over_G, beta, epsilon, Coeff, screen_term, delta_m;
  beta  = 1. + (4./3./a) * (H_conf/H0) * H0rc * (1. + H_conf_prime/2./ (H_conf * H_conf) );
    ...
    source_smooth(x)=source_smooth(x)/(num_pts * num_pts * num_pts);
    source(x)=source(x)/(num_pts * num_pts * num_pts);
    delta_m = (source_smooth(x) - Omega_m0)/Omega_m0;
    epsilon = 8. * H0rc * H0rc * delta_m * Omega_m0 /(9. * a * a * a  * beta * beta);
    screen_term = (sqrt(1 + epsilon) - 1.)/epsilon;
    DeltaG_over_G = (2./3./beta) * screen_term;
    // if(x.coord(0)==15 && x.coord(1)==15 && x.coord(2) ==15)
    // {
    //   cout<<"source: "<<source(x)<<" source_smooth:"<<source_smooth(x)<<endl;
    // }
...
void Smoothing_delta_Gaussian(Field<Cplx> & smoothed_rho, Field<Cplx> & source, const double R_smoothing, const double BoxSize)
{
  rKSite k(smoothed_rho.lattice());
  double k2;

    for (k.first(); k.test(); k.next())
    {
      // k2= 2.0 *(cos(2.0*M_PI*k.coord(0)/BoxSize)
      //         + cos(2.0*M_PI*k.coord(1)/BoxSize)
      //         + cos(2.0*M_PI*k.coord(2)/BoxSize)-3.0);
      smoothed_rho(k) = source(k) * exp(-k2*R_smoothing*R_smoothing/2.0);
      //Tests:
      // cout<<"k: "<<k.coord(0)<<endl;
      // if(k.coord(0) == 0 && k.coord(1) == 0 && k.coord(2) == 0 )
      // {
      //
      //   cout<<"rho: "<< smoothed_rho(k)<<" smooth rho:  "<<smoothed_rho(k)<<" Func:"<<exp(-k2*R_smoothing*R_smoothing/2.0)<<endl;
      //   cout<<"Rsmoothi: "<<R_smoothing<<endl;
      // }
    }
}
