...
template <class FieldType>
void Modified_Gravity_Newtonian_nDGP_screened(Field<FieldType> & phi_prime, Field<FieldType> & source, const double H0rc, const double a, const double H_conf , const double H0, const double H_conf_prime, const double fourpiG, const double Omega_m0, const double dx )
{
  ...
  // double sigma=1.;
    ...
    phi_prime(x) = (  DeltaG_over_G ); // Modified Posson equation

    // }
    // if(x.coord(0)==15 && x.coord(1)==15 && x.coord(2) ==15)
    // {
    // cout<<"density: "<< delta_m <<"DG/G: "<<DeltaG_over_G<<endl;
    // }
    // phi_prime(x) = 10. *exp( -( (x.coord(0) - length/2.)*(x.coord(0) - length/2) + (x.coord(1) - length/2) * (x.coord(1) - length/2) + (x.coord(2) - length/2) * (x.coord(2) - length/2) )/(2.*sigma*sigma)  ); // Modified Posson equation
