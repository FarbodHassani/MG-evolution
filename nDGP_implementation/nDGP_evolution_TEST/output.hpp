...
if (sim.out_pk & MASK_PHI)
  {
    ...
    sprintf(filename, "%s%s%03d_dg.dat", sim.output_path, sim.basename_pk, pkcount);
    writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real) numpts3d * (Real) numpts3d * 2. * M_PI * M_PI , filename, "power spectrum of phi_prime/H (dimensionless)", a, sim.z_pk[pkcount]);
  }
