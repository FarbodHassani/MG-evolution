...
template <class FieldType>
void Modified_Gravity_Newtonian_nDGP_screened(Field<FieldType> & source_mg, Field<FieldType> & source, const double H0rc, const double a, const double H_conf , const double H0, const double H_conf_prime, const double fourpiG, const double Omega_m0, const double dx )
{
	Site x(source_mg.lattice());
  double DeltaG_over_G, beta, epsilon, Coeff;
  beta  = 1. + (4./3./a) * (H_conf/H0) * H0rc * (1. + H_conf_prime/2./ (H_conf * H_conf) );
  Coeff = a * a * a * 9. * beta * beta;
	for (x.first(); x.test(); x.next())
	{
    epsilon = 8. * H0rc * H0rc * source(x);
    // cout<<"Beta: "<<beta<<" T00: "<<source(x)<<" epsilon: "<<epsilon<<endl;

    DeltaG_over_G = (2./3./beta) * (sqrt(1 + epsilon) - 1.)/epsilon;
		source_mg(x) = (1. +  DeltaG_over_G ) * source(x); // Modified Posson equation
