...
template <class FieldType>
void Yukawa_delta(Field<FieldType> & source_screen, Field<FieldType> & source,  const double fR0, const double theta_avg, const double r_th, const double alpha, const double a, const double H_conf , const double H0, const double fourpig,  const double Omega_m0, const double Omega_L0)
{
  ...
  C_2 = Omega_m0 * r_th * r_th * r_th/(4.0 * Omega_L0);
    ...
    if(delta_m>=0)
    raduis = pow(theta_avg/delta_m , 1/3); // Mass at each point which is smoothed one.
    else
    raduis = pow(-theta_avg/delta_m , 1/3); // Mass at each point which is smoothed one.
